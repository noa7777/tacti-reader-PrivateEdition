...
| `G` | 跳转指定页码 |
| `Ctrl + G` | 标定物理页码偏移 |
| `Space` | 跳转到主线页（阅读过的最大页码） |
| `Ctrl + Space` | 将当前页设为主线页 |
| `Ctrl + A` | 单步回退 |
| `Q` ~ `P` | 跳转到对应书签 |
| `Ctrl + Q` ~ `P` | 设置书签 |
| `Alt + Q` ~ `P` | 清除书签 |
...
| `S` | 立即保存状态 |
| `F11` | 全屏 |
|  `Ctrl + 滚轮滚动` | 以鼠标为圆点放大或缩小；你翻下页或按下x会有惊喜 |


批注三种工具（画笔、高亮、文字）均提供 9 色可选，默认均为**纯正红**，这个湊9种颜色太难了他们每三个一组你可以试一下非常盲操（但并没有适配最后两种颜色主题），文字批注默认字号 25pt。

**界面特性**

- **颜色模式**：视图菜单支持切换颜色模式。
- **目录交互**：`L` 键弹出目录对话框，顶部显示书签栏（Q~P），下方为 PDF 目录层级，自动展开并记忆选中节点。
